...
export default function AdminPanel() {
  ...
  const adminFeedback = useStore((s) => s.adminFeedback);
  ...
  const stats = useMemo(() => {
    ...
    const pendingFeedback = adminFeedback.filter((f) => f.status === 'pending').length;
    
    // Average savings rate set by users
    const avgSavingsRate = totalUsers > 0 
      ? adminUsers.reduce((acc, u) => acc + (u.targetSavingsRate || 0), 0) / totalUsers
      : 0;
  ...
  }, [adminUsers, adminFeedback, transactions]);
    ...
    <div className="max-w-5xl mx-auto space-y-6 fade-in select-none">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-surface border border-theme rounded-xl p-4 shadow-sm">
          ...
          <p className="text-xl font-bold text-theme mt-1 mono">
            {adminFeedback.length} <span className="text-[10px] text-muted font-normal">({stats.pendingFeedback} pending)</span>
          </p>
        </div>
        <div className="bg-surface border border-theme rounded-xl p-4 shadow-sm">
      ...
      {activeTab === 'users' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-surface border border-theme rounded-xl p-5 space-y-4">
            ...
            {adminUsers.length === 0 ? (
              <p className="text-xs text-muted text-center py-6">No user accounts registered.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-theme">
                    {adminUsers.map((u) => (
                      <tr key={u._id || u.id} className="hover:bg-surface-2/45 transition-colors">
                        <td className="py-3 flex items-center gap-2 min-w-0">
                          <img 
                            ...
                            alt="" 
                            className="w-6 h-6 rounded-full border border-theme bg-surface-2 shrink-0 object-cover" 
                          />
        ...
        <div className="bg-surface border border-theme rounded-xl p-5 space-y-4">
          ...
          {adminFeedback.length === 0 ? (
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto">
              {adminFeedback.map((ticket) => (
                <div key={ticket._id || ticket.id} className="p-4 border border-theme rounded-xl bg-surface-2 text-xs flex flex-col gap-2">
                    ...
                    {ticket.status === 'pending' ? (
                      <div className="flex gap-2 items-center">
                        <input
                          ...
                          onChange={(e) => setReplies((prev) => ({ ...prev, [ticket._id || ticket.id]: e.target.value }))}
                        ...
                        />
                        <button
                          onClick={() => handleResolveFeedback(ticket._id || ticket.id)}
